    ...
    public partial class Form1 : Form
    {
        ...
        public void checkDB()
        {
            try
            {
                ...
                {
                    sc.Open();
                    MySqlDataAdapter da = new MySqlDataAdapter(@"SELECT * FROM Scope_Of_Drug_Using", sc);
                    DataSet data = new DataSet();
                    da.Fill(data);
                    if (data.Tables[0].Rows.Count == 0)
                    {
                        da = new MySqlDataAdapter(@"INSERT INTO Drug_Category (Name, Description) VALUES (N'Aids', N'For first help')", sc);
                        da.Fill(data);
                    }
                    da = new MySqlDataAdapter(@"select * from Drug_Category where Name = N'Alcohol'", sc);
                    data.Reset();
                    da.Fill(data);
                    if (data.Tables[0].Rows.Count == 0)
                    {
                        da = new MySqlDataAdapter(@"INSERT INTO Drug_Category (Name, Description) VALUES (N'Alcohol', N'For second help')", sc);
                        da.Fill(data);
                    }
                }
            }
        ...
        public void fillCategories()
        {
            try
            {
                ...
                {
                    sc.Open();
                    MySqlDataAdapter da = new MySqlDataAdapter("SELECT NAME FROM Drug_Category", DB);
                    da.Fill(data);
                }
           ...
           // Regex check3 = new Regex("^[(yes)(no)]$");
            ...
            if (gettingCheckBox.Checked)
            {
                if (IDCheckBox.Checked && !CategoryCheckBox.Checked)
                {
                    ...
                    {
                        res = "row(s) selected";
                        //query = "select * from Scope_Of_Drug_Using join Drug_Category On Scope_Of_Drug_Using.Category_ID = Drug_Category.Category_ID";
                        if (int.TryParse(IDTextBox.Text, out id))
                        {
                            query += "select * from Scope_Of_Drug_Using where Scope_Of_Drug_Using.Scope_ID = " + IDTextBox.Text;
                        }
                    }
                    else
                    {
                ...
                else if (CategoryCheckBox.Checked && !IDCheckBox.Checked)
                {
                    ...
                    {
                        res = "row(s) selected";
                        query = $"SELECT * FROM Scope_Of_Drug_Using WHERE Scope_Of_Drug_Using.Category_ID = (SELECT Drug_Category.Category_ID FROM Drug_Category WHERE Drug_Category.Name = N'{CategoryComboBox.SelectedItem}')";
                ...
                { 
                    ...
                    {
                        query += $" where Scope_Of_Drug_Using.Category_ID = (SELECT Drug_Category.Category_ID FROM Drug_Category WHERE Name = N'{CategoryComboBox.SelectedItem}')";
                    }
                }
            }
            else if (addingCheckBox.Checked)
            {
                if (!InstructionRes.Success && !ScopeRes.Success && !LegalityRes.Success && !RecipeRes.Success && CategoryComboBox.Text != "")
                {
                    ...
                    query = "INSERT INTO Scope_Of_Drug_Using (Instruction, Scope_Of_Use, Recipe, Legal, Category_ID) VALUES (N'" + InstructionTextBox.Text + 
                        "',N'" + ScopeTextBox.Text + "',N'" + RecipeTextBox.Text + "',N'" + LegalityTextBox.Text + "',(select Drug_Category.Category_ID from Drug_Category where Name = N'" + CategoryComboBox.Text + "'))";
                }
                else
                {
                    MessageBox.Show("Error!");
            ...
            else if (updatingCheckBox.Checked)
            {
                ...
                {
                    res = "1 row(s) updated";
                    query = "UPDATE Scope_Of_Drug_Using SET Instruction = N'" + InstructionTextBox.Text + "', Category_ID = (select Category_ID from Drug_Category where Name = N'" + CategoryComboBox.Text + "'), " +
            ...
            else if (deletingCheckBox.Checked)
            {
                ...
                {
                    res = "1 row(s) deleted";
                    query = "delete from Scope_Of_Drug_Using where Scope_ID = " + IDTextBox.Text;
                }
                else
                {
                    MessageBox.Show("Error!");
            ...
            if (flag == 0)
            {
                using (MySqlConnection conn = new MySqlConnection(DB))
                {
                    ...
                    {
                        adapt = new MySqlDataAdapter("select * from Scope_Of_Drug_Using join Drug_Category On Scope_Of_Drug_Using.Category_ID = Drug_Category.Category_ID", DB);
                        data = new DataSet();
                        adapt.Fill(data);
                        dataGridView1.DataSource = data.Tables[0];
                        textBoxRes.Text = res;
